...
public class SupplyManagmentDBController {
	...
	public static void getMachineItems(int machineId, ConnectionToClient client) {
		...
		try {
			...
			//   1                  2              3                 4          5                6              7            8     9
			//machine_id,         item_id,    current_amount call_status,   times_under_min, calls_amount, name, item_img_name
			/*(int machineID, int item_id, int currentAmount,int minAmount,call_Status callStatus,
				...
				item= new ItemInMachineEntity(machineId, res.getInt(2),res.getInt(3),ItemInMachineEntity.call_Status.NotOpened , res.getString(8), 0.0, null, null, res.getString(9), res.getInt(6),res.getInt(7));
