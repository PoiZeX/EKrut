...
import ocsf.server.*;
/**
 * The EchoServer class is an implementation of the OCSF library's AbstractServer class.
 * It is a basic echo server that listens for incoming connections from clients and
 * handles the communication between them.
 * The server has a port number which it listens on, a DatabaseEntity that contains
 * the connection details to the database, and an ObservableList of ConnectedClientEntity
 * that stores the clients that are currently connected to the server.
 * The server listens for incoming connections and handles incoming messages.
 * If the message is a string, it will print it to the console. If the message is an instance
 * of the Message class, it will pass it to the MessageHandler.handle() method for further processing.
 * The server also has a start and stop methods, serverStarted() and serverStopped(),
 * which are called by the OCSF library when the server starts and stops.
 * They can be overridden to provide custom functionality, but in this case, it just
 * prints a message to the console indicating that the server has started or stopped.
*/
public class EchoServer extends AbstractServer {
	...
	protected void clientConnected(final ConnectionToClient client) {
	  ...
	  /**
	    * Creates an instance of EchoServer and initializes the port and the database credentials for connection
	...
	public void handleMessageFromClient(Object msg, ConnectionToClient client) {
	    ...
	    * @param connectionStatus the status of the client's connection (Connected/Disconnected)
	    */
    ...
    /**
    *  This method is called when the server has started and is ready to accept connections
    */
    ...
    /**
    * This method is called when the server has been stopped and is no longer accepting connections
    */
